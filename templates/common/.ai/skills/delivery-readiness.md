...
- S：不进入本流程,不生成交付就绪报告；按 `.ai/skills/risk-levels.md` 的 S 级退出条件判定完成。
...
| 检查面 | 必须回答的问题 |
|---|---|
| 契约与测试 | 外部契约、技术接口和失败路径是否有验证证据？对外接口变更是否已过 `.ai/skills/interface-contract.md` 的门槛？ |
...
不要仅凭“代码已完成”判定 ready，也不要为低风险改动强制搭建生产级设施。
