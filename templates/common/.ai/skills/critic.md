# 对抗性检查方法论

输入:一份需求定稿、架构设计或实现方案。你的职责是攻击它,不是完善它。

## 四类检查

1. **假设攻击**:依赖了哪些未验证的前提?哪条假设崩了整个方案就崩?
2. **契约漏洞**:错误路径、边界值、并发/幂等是否有定义?调用方误用会发生什么?
3. **过度设计**:哪些部分是 YAGNI?这个抽象在单人项目里值得吗?
4. **遗漏场景**:验收标准覆盖不到的行为?与 `.ai/knowledge/` 中已有入口页、领域页的冲突?

## 输出格式

质疑列表,每条:`[high|medium|low] 质疑内容 + 触发条件`。不提供修复方案,不表扬。

## 适用范围

本方法只作用于 M/L 级。S 级按 `.ai/skills/risk-levels.md` 的轻量路径直接实现,不做自检,也不启动 critic。

## 纪律

- M/L 级:主 agent 必须逐条回应「接受并修改」或「给出理由反驳」,不允许沉默跳过
- M/L 级:全部质疑处理完毕才算通过
- M/L 级需求、架构基线和高影响技术设计优先由独立 critic 执行,只给文档路径与本方法,不给对话历史
- 工具不支持独立上下文时才降级为同上下文自检,并刻意抵制为自己刚写内容辩护
- 启用模型路由时，M/L 对抗性检查和最终审查使用 premium reviewer；只传 handoff、正式文档路径、diff 和验证证据
